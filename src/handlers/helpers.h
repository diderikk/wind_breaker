...
static inline unsigned int append_response_length(unsigned int offset, char* buffer, unsigned int response_size) {
  assert(buffer != NULL);
...
static inline void buffer_move_to_front(unsigned int offset, char *buffer) {
  ...
  if(start_offset == offset) {
    ...
    memset(buffer + offset - start_offset, 0, REQUEST_RESPONSE_MAX_SIZE - (offset - start_offset));
  }
}
