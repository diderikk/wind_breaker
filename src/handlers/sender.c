...
void *handle_d() {
  ...
  while (!stop()) {
    ...
    unsigned int response_size = combine_chars(session.out_buffer[0],
                                               session.out_buffer[1],
                                               session.out_buffer[2],
                                               session.out_buffer[3]);

    log_info("Sending response to session %d:\n%s",
             session.session->id, session.out_buffer + 4);
    ...
    if (is_ssl && send_return <= 0 &&
      ...
      log_debug("Connection close requested, closing connection on fd: %d, id: %d...", 
