...
// int extract_body(http_request_t* http_request, char * char_data, long content_size);
...
int parse_control_data(http_request_t* http_request, char * raw_control_data){
    ...
    strncpy(method_str, raw_control_data + matches[1].rm_so, matches[1].rm_eo - matches[1].rm_so);
    ...
    // HTTP URI
    strncpy(http_request->uri, raw_control_data + matches[2].rm_so, matches[2].rm_eo - matches[2].rm_so);
    http_request->uri[matches[2].rm_eo - matches[2].rm_so] = '\0';

    // HTTP Version
    strncpy(http_request->version, raw_control_data + matches[3].rm_so, matches[3].rm_eo - matches[3].rm_so);
...
//                printf("Match %d: %.*s\n", i, matches[i].rm_eo - matches[i].rm_so, text + matches[i].rm_so);
//            }
//        }
