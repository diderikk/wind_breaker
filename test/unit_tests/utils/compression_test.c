...
void *compress_decompress_gzip_test() {
  const char *input_data =
      "This is a test string for gzip compression and decompression.";
  ...
  int decompressed_size =
      decompress_gzip(compressed_data, compressed_size, decompressed_data,
...
void *compress_decompress_deflate_test() {
  const char *input_data =
      "This is a test string for deflate compression and decompression.";
  size_t input_size = strlen(input_data) + 1; // Include null terminator
  ...
  // Compress the input data
  int compressed_size =
      compress_deflate(input_data, input_size, compressed_data);
  assert(compressed_size > 0);

  // Decompress the data
  int decompressed_size =
      decompress_deflate(compressed_data, compressed_size, decompressed_data,
                         sizeof(decompressed_data));
  assert(decompressed_size > 0);
...
void *compress_decompress_empty_input_test() {
  ...
  int compressed_size_gzip =
      compress_gzip(input_data, input_size, compressed_data);
  assert(compressed_size_gzip >= 0);

  // Decompress the data with gzip
  int decompressed_size_gzip =
      decompress_gzip(compressed_data, compressed_size_gzip, decompressed_data,
                      sizeof(decompressed_data));
  ...
  int compressed_size_deflate =
      compress_deflate(input_data, input_size, compressed_data);
  assert(compressed_size_deflate >= 0);

  // Decompress the data with deflate
  int decompressed_size_deflate =
      decompress_deflate(compressed_data, compressed_size_deflate,
...
void *compress_decompress_large_input_test() {
  const char *input_data =
      "This is a test string for large input compression and decompression.";
  size_t input_size =
      strlen(input_data) * 100; // Repeat the string to make it large
  ...
  int compressed_size_gzip =
      compress_gzip(large_input_data, input_size + 1, compressed_data);
  assert(compressed_size_gzip > 0);
  ...
  int compressed_size_deflate =
      compress_deflate(large_input_data, input_size + 1, compressed_data);
  assert(compressed_size_deflate > 0);
...
int compression_test() {
  compression_start_case(compress_decompress_gzip_test,
                         "compress_decompress_gzip_test");
  compression_start_case(compress_decompress_deflate_test,
                         "compress_decompress_deflate_test");
  compression_start_case(compress_decompress_empty_input_test,
                         "compress_decompress_empty_input_test");
  compression_start_case(compress_decompress_large_input_test,
                         "compress_decompress_large_input_test");
