...
void *add_and_get_all_test() {
  ...
  for (int i = 1; i < TEST_MAX_SIZE + 1; i++) {
    fd[i - 1] = i;
    pthread_create(&threads[i - 1], NULL, get_session_for_thread_func,
                   &fd[i - 1]);
  }
...
int session_test() {
  ...
  session_start_case(pop_request_by_fd_initial_test,
                     "pop_request_by_fd_initial_test");
  session_start_case(pop_request_by_fd_initial_with_ssl_test,
                     "pop_request_by_fd_initial_with_ssl_test");
  ...
  /*session_start_case(push_request_rejected_test,
   * "push_request_rejected_test");*/
  /*session_start_case(get_session_id_for_thread_test,
   * "get_session_id_for_thread_test");*/
  /*session_start_case(push_request_overflow_test,
   * "push_request_overflow_test");*/
